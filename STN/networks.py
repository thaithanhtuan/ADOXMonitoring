...
class FeatureRegression(nn.Module):
    def __init__(self, input_nc=512,output_dim=6, use_cuda=True):
        ...
        # self.linear = nn.Linear(64 * 3 * 4, output_dim)
        self.linear = nn.Linear(64 * 5 * 7, output_dim)
        self.tanh = nn.Tanh()
        if use_cuda:
            self.conv.cuda()
            self.linear.cuda()
    ...
    def forward(self, x):
        x = self.conv(x)
        # x = x.view(x.size(0), -1)
        x = x.reshape(x.size(0), -1)  # Reshape works without memory constraints
        x = self.linear(x)
        x = self.tanh(x)
        return x
...
class GMM(nn.Module):
    ...
    def __init__(self, opt):
        ...
        # b, h, w, h * w
        self.regression = FeatureRegression(input_nc=609, output_dim=2*3, use_cuda=True)
        self.gridGen = AffineGridGen(opt.fine_height, opt.fine_width)
